...
### 🕹️ Chrome Extension Dashboard (Primary)
The Chrome Extension serves as your primary control center:
...
### 📱 Telegram Bot Commands (Secondary)
You can still interact with the pipeline via Telegram if you are away from your browser:
*   `/queue`: Fetches the current pending queue and allows you to approve/skip videos inline.
*   `/process [url] [category]`: Bypasses the queue and instantly analyzes a video.
*   `/add [url] [category]`: Adds a new YouTube channel to track.
*   `/channels`: Lists all currently tracked channels.
*   `/cost`: Retrieves your daily API usage costs.
